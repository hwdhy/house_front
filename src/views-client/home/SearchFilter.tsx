...
const SearchFilter = (props) => {
  ...
  const [priceType, setPriceType] = useState<any>(1);

  const [priceMinInput, setPriceMinInput] = useState<any>();

  const [priceHW, setPriceHW] = useState<any>();
  ...
  const [distanceInputValue, setDistanceInputValue] = useState<any>();
  ...
  const handleAddressChange = (location, distance) => {
    if (location) {
      onChange({ regionEnName: null, keyword: null, distanceSearch: { lon: location.lng, lat: location.lat, distance: distance } });
    }
  };
      ...
      <Row gutter={RowGutter} style={{ lineHeight: "32px" }}>
                        ...
                        <Col span={22} className="option">
                          <Skeleton active={true} loading={searchTypeLoading} paragraph={{ rows: 1 }} title={false}>
                            <OptionSpanRadioGroup value={searchParams.regionEnName} onChange={(value) => {
                              initDistance();
                              onChange({ regionEnName: value, keyword: null, distanceSearch: null })
                            }}>
                              {
        ...
        <Col span={22} className="option">
          <OptionSpanRadioGroup value={searchParams.rentWay} onChange={value => onChange({ rentWay: value })}>
            ...
            <OptionSpanRadio value={2}>分租</OptionSpanRadio>
          </OptionSpanRadioGroup>
        </Col>
        <Col span={2} className="title">
          租金
        ...
        <Col span={22} className="option">
          <OptionSpanRadioGroup value={priceType === 1 && (searchParams.priceHW)} onChange={value => {
            if (value) {
              setPriceType(1);
              setPriceMinInput(null);
              setPriceMaxInput(null);
              setPriceHW(value);
              onChange({ priceType: value, priceMin: null, priceMax: null });
              ...
              setPriceMaxInput(null);
              setPriceHW(null);
              onChange({ priceMin: null, priceMax: null, priceType: null });
            }
          }}>
            <OptionSpanRadio value="1">10元/㎡以下</OptionSpanRadio>
            <OptionSpanRadio value="2">10-15元/㎡</OptionSpanRadio>
            <OptionSpanRadio value="3">15-20元/㎡</OptionSpanRadio>
            <OptionSpanRadio value="4">20-25元/㎡</OptionSpanRadio>
            <OptionSpanRadio value="5">25-30元/㎡</OptionSpanRadio>
            <OptionSpanRadio value="6">35元/㎡以上</OptionSpanRadio>
          </OptionSpanRadioGroup>
          <OptionSpan>
          ...
          <OptionSpan checked={true} disabled={isNumberEmpty(priceMinInput) || isNumberEmpty(priceMaxInput)} onClick={() => {
            if (!isNumberEmpty(priceMinInput) && !isNumberEmpty(priceMaxInput)) {
              ...
              onChange({ priceMin: min, priceMax: max, priceType: "0" });
            }
          }}>确定</OptionSpan>
        </Col>
        <Col span={2} className="title">房源特色</Col>
